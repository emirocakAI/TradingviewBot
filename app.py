import os
import subprocess
import sys

# --- KRİTİK: KÜTÜPHANEYİ ZORLA YÜKLE ---
# Bu blok, requirements.txt hata verse bile kütüphaneyi manuel kurar.
try:
    from playwright_stealth import stealth_async
except ImportError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "playwright-stealth"])
    from playwright_stealth import stealth_async

import streamlit as st
import asyncio
from playwright.async_api import async_playwright
from PIL import Image, ImageDraw, ImageFont

# --- SİSTEM AYARLARI ---
# Tarayıcı eksikse yükle
if not os.path.exists("/home/appuser/.cache/ms-playwright"):
    os.system("playwright install chromium")

FONT_PATH = "Outfit-VariableFont_wght.ttf"

def get_styled_fonts():
    try:
        # Şirket İsmi: Semibold (24)
        font_semibold = ImageFont.truetype(FONT_PATH, 24)
        # Getiri: Black (24) - Black etkisi için puntoyu 26 yapalım
        font_black = ImageFont.truetype(FONT_PATH, 26) 
        return font_semibold, font_black
    except:
        return ImageFont.load_default(), ImageFont.load_default()

# --- GRAFİK YAKALAMA ---
async def capture_chart(ticker):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True, args=["--no-sandbox"])
        context = await browser.new_context(viewport={'width': 1200, 'height': 800})
        page = await context.new_page()
        await stealth_async(page)
        
        # TradingView Sembol Sayfası
        await page.goto(f"https://www.tradingview.com/symbols/BIST-{ticker}/", wait_until="networkidle")
        await asyncio.sleep(5)
        
        path = f"{ticker}.png"
        await page.screenshot(path=path)
        await browser.close()
        return path

# --- GÖRSEL İŞLEME ---
def process_image(path, company, date_range, yield_val):
    img = Image.open(path)
    draw = ImageDraw.Draw(img)
    f_semi, f_black = get_styled_fonts()
    
    # 1. Şirket İsmi (Outfit Semibold 24)
    draw.text((50, 50), company.upper(), font=f_semi, fill="white")
    
    # 2. Getiri (Outfit Black 24)
    draw.text((50, 90), f"{date_range} Getiri: %{yield_val}", font=f_black, fill="#00FF00")
    
    final_path = f"final_{ticker}.png"
    img.save(final_path)
    return final_path

# --- ARAYÜZ ---
st.title("FinansZone Grafik Botu")

ticker = st.text_input("Hisse Kodu:", "SASA").upper()
company = st.text_input("Şirket İsmi:", "SASA POLYESTER")
date_range = st.text_input("Tarih Aralığı:", "01-26 Mart")
yield_val = st.text_input("Getiri Oranı:", "12,45")

if st.button("Raporu Oluştur"):
    with st.spinner("Hazırlanıyor..."):
        try:
            raw_path = asyncio.run(capture_chart(ticker))
            final_path = process_image(raw_path, company, date_range, yield_val)
            st.image(final_path)
            with open(final_path, "rb") as f:
                st.download_button("İndir", f, "rapor.png")
        except Exception as e:
            st.error(f"Hata: {e}")
